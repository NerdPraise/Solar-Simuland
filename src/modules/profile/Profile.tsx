...
const Profile: FC<ProfileProps> = ({ projects, getProjects }) => {
  ...
  const projectHtmlView = projects.map((project, index) => {
    return (
      <Col md={{ span: 7 }} sm={{ span: 24 }} key={index}>
        <div
          className="cardWrapper mb-9"
          onClick={() =>
            window.location.replace(`/projects/${project.load_profile}`)
          }
        >
          <Card
        ...
        <Row justify="space-between">
          {projects.length > 0 ? (
            ...
            <Col md={{ span: 24 }} sm={{ span: 24 }}>
              <div className="text-center text-gray-300 text-2xl border-2 border-blue-100 py-3 rounded-lg font-sans">
